/**
 * In-app changelog data.
 *
 * Source of truth for the landing page lives in
 * `apps/landing/src/lib/releases.ts` — this file mirrors that data in the
 * shape the in-app Changelog view expects. Keep the two in sync when shipping
 * a new release; the landing entry remains the canonical public copy.
 *
 * Categories map to `PillTag` variants used in the view:
 *   - `feature`  → accent
 *   - `fix`      → green
 *   - `polish`   → blue
 *   - `security` → orange
 ...
 *   - `bot`      → muted
 */
export type ChangelogCategoryKind =
  | 'feature'
  | 'fix'
  | 'polish'
  | 'security'
...
export interface ChangelogCategory {
  ...
  /** Uppercase label shown in the pill tag next to the list. */
  label: string;
  /** Bullet list of user-facing changes. Plain strings only — keep copy non-technical. */
  entries: string[];
}

export type ChangelogReleaseType = 'major' | 'minor';
