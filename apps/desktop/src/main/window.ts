...
function setupContentSecurityPolicy(isDev: boolean, backendPort: number): void {
  ...
  session.defaultSession.webRequest.onHeadersReceived(urlFilter, (details, callback) => {
    ...
    const cspDirectives = [
      ...
      isDev
        ? `connect-src 'self' http://localhost:${VITE_DEV_PORT} ws://localhost:${VITE_DEV_PORT} http://localhost:${backendPort} ws://localhost:${backendPort} http://127.0.0.1:${backendPort} ws://127.0.0.1:${backendPort} https://*.anilist.co`
        : `connect-src 'self' http://localhost:${backendPort} ws://localhost:${backendPort} http://127.0.0.1:${backendPort} ws://127.0.0.1:${backendPort} https://*.anilist.co`,
